...
def test_get_oauth1_token_retries_on_transient_errors(
    client: Client, status_code: int
):
    from unittest.mock import MagicMock, call, patch

    mock_retry_resp = MagicMock()
    mock_retry_resp.ok = False
    mock_retry_resp.status_code = status_code  # noqa: S105

    mock_resp_200 = MagicMock()
    mock_resp_200.ok = True
    mock_resp_200.text = "oauth_token=tok&oauth_token_secret=sec"  # noqa: S105
    ...
    def mock_get(*a, **kw):
        call_count["n"] += 1
        if call_count["n"] < 3:
            return mock_retry_resp
        return mock_resp_200

    with (
        patch.object(
            sso.GarminOAuth1Session,
            "__init__",
            lambda *a, **kw: None,
        ),
...
def test_get_oauth1_token_raises_after_retries(client: Client):
    from unittest.mock import MagicMock, call, patch
